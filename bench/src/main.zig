...
//! Discovers frameworks/*/Dockerfile and generates compose on the fly.
...
pub fn main(init: std.process.Init) !void {
    ...
    for (frameworks) |fw| {
        ...
        // Run each scenario with a fresh container. Zinc's aio/io_uring path can
        // wedge after a heavy plaintext blast; recreating keeps json measurable.
...
fn findRepoRoot(arena: std.mem.Allocator, io: Io) ![]const u8 {
    ...
    while (true) {
        ...
        const bench = try std.fs.path.join(arena, &.{ dir, "bench", "Dockerfile.bench" });
